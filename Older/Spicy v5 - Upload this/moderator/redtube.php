...
$thumb = $video['video']['thumbs']['1']['src'];
}else {
$thumb = $video['video']['default_thumb'];
}
if(!validateRemote($thumb)) {
$thumb = str_replace(array("ei-ph.rdtcdn.com/m=","ci-ph.rdtcdn.com/m="),"thumbs-cdn.redtube.com/m=",$thumb);	
}
if(substr($thumb, 2) == "//")
{
$thumb = 'https:'.$thumb;
...
<div class="row-fluid">
	...
	<div class="span4">
		<input class="span12" name="endpage" type="text" value="5"><span class="help-block k align-right">Which page to end push? </span>
	</div>
</div>
	</div>
	</div>		
...
<div class="controls">
<?php
echo '<select id="redsTags" data-placeholder="Redtube tags" name="tags" id="clear-results" class="select" tabindex="2">';
...
<script>
var sxurl = '<?php echo site_url().ADMINCP;?>/cache/tags.json';
...
$.each(result, function(key, value) {
   ...
   $('#redsTags').append($('<option>', {
    value: value['tag']['tag_name'],
    text: value['tag']['tag_name']
}));
});
    });
</script>					
