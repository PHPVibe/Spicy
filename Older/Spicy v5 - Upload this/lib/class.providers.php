...
$id = end($ids);
$link = 'http://api.tube8.com/api.php?action=getvideobyid&output=json&thumbsize=big&video_id='.$id;
$html = $this->getDataFromUrl($link);
...
case 'youporn':
$videoId = $this->getVideoId("watch/","/");
$link = 'http://www.youporn.com/api/webmasters/video_by_id/?output=json&thumbsize=big&video_id='.$videoId;
$html = $this->getDataFromUrl($link);
$video             = json_decode($html, true);
$video = $video['video'];
$video['description'] = $video['title'];
...
$ids = explode("/",rtrim($this->link,'/'));
$videoId = end($ids);
$link = 'http://fantasti.cc/wm/getVideoById.json?thumbsize=big&video_id='.$videoId;
$html = $this->getDataFromUrl($link);
$video             = json_decode($html, true);
$video = $video['video'];
$video['description'] = $video['title'];
